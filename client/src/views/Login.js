...
function Login() {
  ...
  const handleSubmit = (event) => {
    ...
    AuthService.register(data.get('first_name'), data.get('last_name'), data.get('email'), data.get('password'))
    .then(response => {
      if(response.success){
  ...
  return (
    <Container>
        ...
        <form onSubmit={handleSubmit}>
            ...
            <button type="submit" className="btn bt-bgn-blue w-100 text-light mt-3 rounded">Login</button>
